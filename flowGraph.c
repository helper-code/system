...
int leaders[MAX] = {0}; // 1 if instruction is a leader
...
void identify_leaders() {
    leaders[0] = 1; // first instruction is a leader
    ...
    for (int i = 0; i < total; i++) {
        if (strstr(tac[i], "goto")) {
            ...
            if (target_index != -1) {
                leaders[target_index] = 1; // target is leader
            }
            if (i + 1 < total)
                leaders[i + 1] = 1; // next instruction is also leader
        }
    }
